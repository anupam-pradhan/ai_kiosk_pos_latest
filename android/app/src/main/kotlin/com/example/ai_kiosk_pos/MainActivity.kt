...
import com.example.ai_kiosk_pos.printer.PrinterV2Manager
...
class MainActivity : FlutterActivity(), TerminalListener, TapToPayReaderListener {
  ...
  companion object {
    ...
    private const val PRINTER_CHANNEL_NAME = "kiosk.printer.v2"
  ...
  private lateinit var printerManager: PrinterV2Manager
  ...
  override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
    ...
    val printerChannel = MethodChannel(flutterEngine.dartExecutor.binaryMessenger, PRINTER_CHANNEL_NAME)

    // Printer v2 is isolated from Stripe/Tap-to-Pay. It transports raw bytes only.
    printerManager = PrinterV2Manager(this)
    ...
    printerManager.jobSender = { event ->
      printerChannel.invokeMethod("onPrinterJobChanged", event)
    }

    printerChannel.setMethodCallHandler { call, result ->
      val args = call.arguments as? Map<*, *> ?: emptyMap<Any, Any>()
      when (call.method) {
        "printerStatus" -> printerManager.status(result)
        "printerScan" -> printerManager.scan(result)
        "printerSave" -> printerManager.save(args, result)
        "printerForget" -> printerManager.forget(result)
        "printerTest" -> printerManager.test(result)
        "printerPrint" -> printerManager.printRaw(args, result)
        "printerDrawer" -> printerManager.drawer(result)
        "printerOpenSettings" -> {
          when ((args["target"] ?: args["type"])?.toString()) {
            "bluetooth" -> openBluetoothSettings()
            "usb" -> openUsbSettings()
            else -> openAppSettings()
          }
          printerManager.status(result)
        }
    ...
    channel.setMethodCallHandler { call, result ->
      ...
      when (call.method) {
        ...
        // ── Printer Operations ──
        "scanPrinters"        -> printerManager.scan(result)
        "connectPrinter"      -> printerManager.save(args, result)
        "disconnectPrinter"   -> printerManager.forget(result)
        "getPrinterStatus"    -> printerManager.status(result)
        "printReceipt"        -> result.success(mapOf(
          "ok" to false,
          "errorCode" to "PRINTER_DEPRECATED",
          "message" to "PRINT_RECEIPT is deprecated. Use PRINTER_PRINT with web-generated bytes."
        ))
        "printKot"            -> result.success(mapOf(
          "ok" to false,
          "errorCode" to "PRINTER_DEPRECATED",
          "message" to "PRINT_KOT is deprecated. Use PRINTER_PRINT with web-generated bytes."
        ))
        "printReport"         -> result.success(mapOf(
          "ok" to false,
          "errorCode" to "PRINTER_DEPRECATED",
          "message" to "Native report printing is not supported in printer v2."
        ))
        "testPrint"           -> printerManager.test(result)
        "printRaw"            -> printerManager.printRaw(args, result)
        "updatePrinterSettings" -> printerManager.status(result)
